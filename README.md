...
* **Dynamic Firewall Rules:** Loads blocking rules from an external text file to block specific domains without recompilation.
...
1. **PCAP Reader:** Reads raw binary data from standard `.pcap` files.
2. **Load Balancers (LB):** Calculates a hash based on the packet's Source IP, Destination IP, Source Port, Destination Port, and Protocol (5-Tuple).
3. **Thread-Safe Queues:** Safely passes parsed `PacketJob` objects from LBs to FPs.
4. **Fast Path Processors (FP):** Pops packets from the queue, updates the state in the `ConnectionTracker`, performs SNI extraction, and checks the `RuleManager`.
5. **Output Writer:** Forwards benign packets to a new output `.pcap` file and drops blocked packets.
